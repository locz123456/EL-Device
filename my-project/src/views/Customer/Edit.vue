  ...
  <v-dialog v-model="dialog" max-width="640px">
    <v-card flat tile>
      <v-card-title class="primary headline">
        <span style="color: white">Update Customer</span>
      </v-card-title>
      <v-card-text>
        <v-form ref="form">
          <v-container>
      ...
      <v-card-actions>
        ...
        <v-btn color="grey darken-1" class="mr-2" dark @click="$emit('close')">
          Cancel </v-btn
        ><v-btn color="primary" class="mr-4" dark @click="updateCustomer">
          Save
        </v-btn>
      </v-card-actions>
    </v-card>
...
export default {
  ...
  methods: {
    updateCustomer() {
      axios
        .put("https://localhost:44371/api/Customer/UpdateCustomer", this.data)
        .then((response) => {
